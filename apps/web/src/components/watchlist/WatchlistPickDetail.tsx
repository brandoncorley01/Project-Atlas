...
export function WatchlistPickDetail({
  ...
  return (
    <div className="mt-3 space-y-3 border-t border-border/60 pt-3">
      ...
      {!loading && sports && <SportsSignalCard row={sports} rank={1} hideSelection />}
      {!loading && stock && <StockSignalCard row={stock} rank={1} showChart />}
      {!loading && option && <OptionSignalCard row={option} rank={1} />}
      {!loading && parlay && <ParlayCard row={parlay} rank={1} />}
