...
def _jwt_role(token: str) -> str | None:
    """Read the unverified JWT role claim (Supabase keys are JWTs)."""
    try:
...
def is_real_service_role_key(key: str | None = None) -> bool:
    """True when SUPABASE_SERVICE_ROLE_KEY is a real service_role JWT (not the anon key)."""
...
def get_service_db() -> SupabaseClient:
    ...
    if not is_real_service_role_key(key):
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=(
                "SUPABASE_SERVICE_ROLE_KEY is missing or set to the anon key. "
...
def get_write_db(user_access_token: str) -> SupabaseClient:
    ...
    Prefer the real service role (bypasses RLS after the request already verified the user).
    Fall back to the caller's JWT when the service role key is missing/misconfigured.
    """
