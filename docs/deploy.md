...
| Still seeing old sports UI / few leagues | Confirm Vercel + Render redeployed latest `main`. Keep credit-safe defaults: `ODDS_SCAN_SCOPE=priority`, `ODDS_MAX_SPORTS_PER_SCAN=4`, `ODDS_INCLUDE_FUTURES_ON_LIVE=false`, `ODDS_MIN_CREDITS_RESERVE=5`, `ATLAS_EXPERT_INTELLIGENCE_ENABLED=true`, then **Manual Deploy**. On phone: hard-refresh, prefer **Rescore** (0 Odds credits) when cache is warm; **Fetch live odds** pulls FanDuel/DraftKings for ~4 US-core leagues and OpenAI ranks the slate. |
