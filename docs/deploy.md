...
| Still seeing old sports UI / few leagues | Confirm Vercel + Render redeployed latest `main`. Keep credit-safe defaults: `ODDS_SCAN_SCOPE=priority`, `ODDS_MAX_SPORTS_PER_SCAN=8`, `ODDS_INCLUDE_FUTURES_ON_LIVE=false`, `ODDS_MIN_CREDITS_RESERVE=25`, `ODDS_SEARCH_PROP_EVENTS=2`, `ODDS_INSIGHT_PROP_EVENTS=2`, `ATLAS_EXPERT_INTELLIGENCE_ENABLED=true`, then **Manual Deploy**. On phone: hard-refresh, prefer **Rescore** (0 Odds credits) when cache is warm; **Fetch live odds** pulls FanDuel/DraftKings for ~8 US+global leagues (in-season essentials pinned) and OpenAI ranks the slate. Search can live-seed books when the cache misses. |
